module Dapper.FSharp.SQLiteTests.SelectTests
...
let testsDebug (crud:ICrud) (init:ICrudInitializer) = testList "DEBUG" [
    ...
    testTask "Selects with skipTake parameter" {
        ...
        Expect.equal 6L (fromDb |> Seq.head |> (fun (x:Persons.View) -> x.Position)) ""
...
let testsBasic (crud:ICrud) (init:ICrudInitializer) = testList "SELECT" [
    ...
    testTask "Selects by NOT IN where condition" {
        ...
        let! fromDb =
            select {
                for p in personsView do
                where (isNotIn p.Position [1L;2L;3L])
                orderBy p.Position
            } |> crud.SelectAsync<Persons.View>
        Expect.equal (rs |> List.find (fun x -> x.Position = 4L)) (Seq.head fromDb) ""
        Expect.equal (rs |> List.find (fun x -> x.Position = 10L)) (Seq.last fromDb) ""
    ...
    testTask "Selects by IS NULL where condition" {
        ...
        Expect.equal (rs |> List.find (fun x -> x.Position = 2L)) (Seq.head fromDb) ""
    ...
    testTask "Selects by IS NOT NULL where condition" {
        ...
        Expect.equal (rs |> List.find (fun x -> x.Position = 1L)) (Seq.head fromDb) ""
        Expect.equal 5 (Seq.length fromDb) ""
    }
    ...
    testTask "Selects by UNARY NOT where condition" {
        ...
        let! fromDb =
            select {
                for p in personsView do
                where (not(p.Position > 5L && p.DateOfBirth = None))
                orderBy p.Position
            } |> crud.SelectAsync<Persons.View>
    ...
    testTask "Selects by multiple where conditions" {
        ...
        let! fromDb =
            select {
                for p in personsView do
                where (p.Position > 2L && p.Position < 4L)
            } |> crud.SelectAsync<Persons.View>
    ...
    testTask "Selects with order by" {
        ...
        Expect.equal 10L (fromDb |> Seq.head |> (fun (x:Persons.View) -> x.Position)) ""
    }
    ...
    testTask "Selects with two inner joins - 1:1" {
        ...
        Expect.equal 10L (Seq.length fromDb) ""
    ...
    testTask "Selects with two inner joins - 1:N" {
        ...
        Expect.equal 3L (Seq.length fromDb) ""
        Expect.equal (persons.Head, dogs.Head, weights.Head) (Seq.head fromDb) ""
    }
    ...
    testTask "Selects with two left joins" {
        ...
        Expect.equal 16L (Seq.length fromDb) ""
    }
]