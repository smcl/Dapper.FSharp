module Dapper.FSharp.SQLiteTests.InsertTests
...
let testsDebug (crud:ICrudOutput) (init:ICrudInitializer) = testList "DEBUG" [
    ...
    testTask "Inserts and outputs subset of single record columns" {
        ...
        Expect.equal r.Position (Seq.head fromDb |> fun (p:{| Position:int64 |}) -> p.Position) ""
    }
]
...
let testsOutput (crud:ICrudOutput) (init:ICrudInitializer) = testList "INSERT OUTPUT" [
    ...
    testTask "Inserts row with Some value and outputs record" {
        do! init.InitPersons()
        let r = Persons.View.generate 1 |> List.head |> fun p -> { p with DateOfBirth = Some (System.DateTime.UtcNow.ToString()) }
        let! fromDb =
            insert {
                into personsView
                value r
