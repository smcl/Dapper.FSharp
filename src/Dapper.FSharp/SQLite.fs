...
module private Evaluators =
    ...
    let evalPagination (pag:Pagination) =
        ...
        | { Take = None; Skip = o } -> sprintf "LIMIT -1 OFFSET %i" o
    ...
    let evalInsertQuery fields outputFields (q:InsertQuery<_>) =
        ...
        | outputFields ->
            let outputFieldNames = outputFields |> String.concat ", "
    ...
    let evalUpdateQuery fields outputFields meta (q:UpdateQuery<'a>) =
        ...
        let returning = 
            ...
            | [] ->
                ""
            | outputFields -> 
                outputFields |> String.concat ", "
        if returning.Length > 0 then sb.Append (sprintf " RETURNING %s" returning) |> ignore
        sb.ToString()
